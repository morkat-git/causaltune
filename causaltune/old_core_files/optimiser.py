...
class CausalTune:
    ...
    #             if self.metric in ["energy_distance", "psw_energy_distance", "frobenius_norm", "codec"]
    ...
    # def _tune_with_config(self, config: dict) -> dict:
    #     print(f"Starting estimation for config: {config}")
    #     estimates = Parallel(n_jobs=2, backend="threading")(
    #         delayed(self._estimate_effect)(config["estimator"]) for i in range(1)
    #     )[0]
        
    #     print(f"Estimation complete. Results: {estimates}")
        
    #     if "exception" not in estimates:
    #         est_name = estimates["estimator_name"]
    #         print(f"Processing results for estimator: {est_name}")
            
    #         if "estimator" in estimates:
    #             print(f"Estimator object present for {est_name}")
    #             self._best_estimators[est_name] = (
    #                 estimates[self.metric],
    #                 estimates["estimator"] if self._settings["store_all"] else estimates.pop("estimator")
    #             )
    #         else:
    #             print(f"Warning: No estimator object found for {est_name}")
    #     else:
    #         print(f"Exception occurred during estimation: {estimates['exception']}")
    ...
    def _tune_with_config(self, config: dict) -> dict:
        ...
        if "exception" not in estimates:
            ...
            if est_name not in self._best_estimators:
                self._best_estimators[est_name] = (np.inf if self.metric in ["energy_distance", "psw_energy_distance", "frobenius_norm", "codec", "policy_risk"] else -np.inf, None)
