...
class Scorer:
    ...
    def get_propensity_weights(self, df: pd.DataFrame, treatment_col: str, clip: float = 0.05):
        try:
            ...
            if propensity_scores.shape[1] == 2:  # Binary treatment
                propensity_scores = propensity_scores[:, 1]
                weights = np.where(df[treatment_col] == 1,
                                1 / propensity_scores,
                                1 / (1 - propensity_scores))
            else:  # Multi-valued treatment
                treatment_values = df[treatment_col].unique()
                weights = np.zeros(len(df))
                for i, value in enumerate(treatment_values):
                    mask = df[treatment_col] == value
                    weights[mask] = 1 / propensity_scores[mask, i]
            
            weights = np.clip(weights, 1/clip, 1/(1-clip))
        except AttributeError:
            # If propensity scores are not available (e.g., in RCT), use equal weights
    ...
    def energy_distance_score(
        ...
        #print(Y0X, " ----- ", split_test_by)

        YX_1 = Y0X[Y0X[split_test_by] == 1]
        YX_0 = Y0X[Y0X[split_test_by] == 0]
        select_cols = estimate.estimator._effect_modifier_names + ["yhat"] #+ ["y_factual"]
    ...
    # ) -> float:
    #     """Calculate adapted Frobenius norm-based score between treated and controls."""
    #     try:
    ...
    #     weights = self.get_propensity_weights(df, split_test_by)
    #     print("WEIGHTS: ", weights)

    #     # Ensure both datasets have the same number of rows
    #     min_rows = min(len(YX_1), len(YX_0))
    #     YX_1 = YX_1.iloc[:min_rows]
    #     YX_0 = YX_0.iloc[:min_rows]
    #     weights = weights[:min_rows]

    #     # Calculate the difference matrix with weights
    #     D = (YX_1[select_cols].values - YX_0[select_cols].values) * weights.reshape(-1, 1)

    #     # Compute Frobenius norm
    #     frobenius_norm = np.linalg.norm(D, ord='fro')

    #     # Normalize by sqrt(n * p)
    #     n, p = D.shape
    ...
    def frobenius_norm_score(
        ...
        Returns:
            float: Adapted Frobenius norm-based score, or np.inf if calculation is not possible
        """
        try:
            cate_estimates = estimate.estimator.effect(df)
        except AttributeError:
    ...
    def policy_risk_score(
        ...
        if np.std(cate_estimate) <= sd_threshold:
            return 0  # Return a 0 score for consistently constant CATE estimates        
        ...
        # Calculate Policy Risk using the RCT subset
    ...
    def make_scores(
        ...
        if self.problem == "backdoor":
            ...
            if "norm_erupt" in metrics_to_report:
                ...
                print("norm_erupt: ", norm_erupt_score)
            ...
            if "prob_erupt" in metrics_to_report:
                ...
                print("prob_erupt: ", prob_erupt_score)
            
            if "frobenius_norm" in metrics_to_report:
                print("COMPUTING FROBENIUS")
                #out["frobenius_norm"] = Scorer.frobenius_norm_score(estimate, df)
                out["frobenius_norm"] = self.frobenius_norm_score(estimate, df)
                #print("frobenius_norm: ", Scorer.frobenius_norm_score(estimate, df))
